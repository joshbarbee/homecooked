...
from homecooked.common.exceptions import InvalidRoute, InvalidRouteParameter, InvalidPostware
...
class RouteNode():
    def __init__(self, route, methods = None, handler = None, populate_request = False) -> None:
        if isinstance(methods, list) and len(methods) > 0 and not isinstance(methods[0], HTTPMethods):
            methods = [HTTPMethods(method) for method in methods if not isinstance(method, HTTPMethods)]
...
class PathNode():
    def __init__(self, path, route = None, error_handlers = None, prewares = None, postwares = None) -> None:
            ...
            if len(params) > 0:
                for param, _type in params:
                    if not param.isidentifier():
                        raise InvalidRouteParameter('Invalid parameter name in route: ' + self.path)
                    
                    self.params[param] = types[_type]
                    self.formatted_path = re.sub(match_regex, type_regexes[_type], self.formatted_path, count = 1)
    ...
    def get_values(self, path):
        ...
        for i, value in enumerate(values[0] if isinstance(values[0], tuple) else values):
...
class RouteTree():
    root = PathNode([RouteNode('/')])
    
    @classmethod
    def add(cls, route: RouteNode, prewares = None, postwares = None) -> None:
        current = cls.root
        if route.route != '/':
            for path in route.paths:
                if path not in current.children:
                    current.children[path] = PathNode(path)
                    current.children[current.children[path].formatted_path] = current.children[path]
    ...
    @classmethod
    def add_error_handler(cls, code, route, callable, populate_request = False) -> None:
        if route == '/':
            cls.root.error_handlers[code] = (callable, populate_request)
            return
        current = cls.root
    ...
    @classmethod
    def get(cls, route: str, method : HTTPMethods) -> Union[RouteNode, None]:
        current = cls.root
    ...
    @classmethod
    def get_error_handler(cls, code, route):
        current = cls.root
        if route == '/':
            handler = current.error_handlers.get(code)
            return handler if handler is not None else (default_error_handlers[code], False)
        for path in [p for p in route.split('/')[1:] if p]:
            if path not in current.children:
